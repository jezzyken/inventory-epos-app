    ...
    <v-card class="mt-2">
      <v-card-title class="py-2">
        <v-row align="center" no-gutters>
          <v-col cols="12" sm="4">
            <v-text-field v-model="search" hide-details dense outlined placeholder="Search stocks..."
              prepend-inner-icon="mdi-magnify" clearable></v-text-field>
          </v-col>
          <v-spacer></v-spacer>
          <v-col cols="auto">
            <v-btn color="primary" :to="{ name: 'AddStock' }" depressed class="ml-2">
      ...
      <v-data-table :headers="headers" :items="desserts" :loading="isLoading" :search="search" :items-per-page="10"
        :footer-props="{
          ...
          'items-per-page-text': 'Stocks per page',
          'page-text': '{0}-{1} of {2}',
        ...
        <template v-slot:item.notes="{ item }">
          ...
          <span v-else class="text-red">No notes available</span>
        </template>
        ...
        <template v-slot:item.status="{ item }">
          <v-chip small :color="item.status === 'Completed' ? 'success' : 'warning'" text-color="white">
            {{ item.status }}
          </v-chip>
        </template>
...
export default {
  ...
  methods: {
    ...
    handleAction(action, item) {
      switch (action) {
        case "Edit":
          this.$router.push({ name: "EditStock", params: { id: item._id } });
          break;
        case "Delete":
