<template>
  <v-container>
    <ViewDeliveryDialog ref="product" :items="selectedItems" @get-items="initialize" />
    <v-data-table :headers="headers" :items="desserts" :loading="isLoading" class="elevation-1 mt-4" :search="search">
      <template v-slot:[`item.status`]="{ item }">
        <v-chip dark label :color="item.status === 'delivered'
          ? 'light-green darken-1'
          : 'amber darken-1'
          " small class="mr-1 text-uppercase">
          {{ item.status }}
        </v-chip>
      </template>
